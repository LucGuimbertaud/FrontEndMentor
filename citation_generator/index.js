...
let text_citation = document.querySelector('#citation');
let text_autor = document.querySelector('#auteur');

// Change citation on cick of button 

button.addEventListener('click', () => {
    text_citation.textContent = newCitation()[0];
    text_autor.textContent = newCitation()[1];
});


// Function that return randomly a citation

function newCitation(){
    return citations[Math.floor(Math.random() * (citations.length - 1))];
}